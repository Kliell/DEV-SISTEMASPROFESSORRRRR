...
def mudar_status(
    ...
    for i,t in enumerate (banco):
        if t.id ==tarefa_id: 
            if t.status ==StatusEnum.cancelada and dados.status !=StatusEnum.cnacelada:
                raise HTTPException(status_code=404, detail='Tarefas Canceladas não podem ser alteradas')
                atual= t.model_dump()
